...
from warehouse_msgs.msg import Rack, RackArray
...
class MapVisualizer(Node):
    ...
    def map_callback(self, msg: OccupancyGrid):
        ...
        # -------------------------------------------------------------------
        # STEP 2: Exhaustive rack candidate search 
        # -------------------------------------------------------------------
        shelves = []
        # rack dims in meters
        L_min, L_max = 0.8, 1.0  # long side in meters
        W_min, W_max = 0.3, 0.4  # short side in meters
        ...
        for s in shelves:
            ...
            box_color = 'red' if abs(angle_deg - 90) <= 25 else 'blue'
            if box_color == 'red':
        ...
        # compute long-side angles (rack orientation)
            #self.get_logger().info(f"Detected {len(candidates)} rack candidates.")
            ...
            #self.get_logger().info(f"Unique rack candidates after filtering: {len(unique_candidates)}")
        ...
        plt.title("rack Detection (Red=Rectangular, Blue=Skewed)")
        plt.pause(0.01)
